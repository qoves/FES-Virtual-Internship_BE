...
exports.LogicalAnlysis = async (req, res) => {
  try {
    ...
    var Cosmeticdata = CosmeticConcerns.filter(
      (x) => x.Prediction == Math.max(...Cosmetic)
    );
    ...
    if (!userdata) {
      ...
      await LogicalAnalysis.findOneAndUpdate(
        ...
        .then((result) => {
          return res.status(200).json({
            code: 200,
            message: "LogicalAnalysis Update Successfully",
            result,
          });
        })
        .catch((error) => {
...
exports.LogicalResponse = async (req, res) => {
  try {
    ...
    let skindata2 = data.SkinType.find(
      (data) => data.Name == "FitzpatrickSkinType"
    );
    const CosmeticData = data.CosmeticConcerns.map((x) => x.Prediction);
    var Cosmetics = data.CosmeticConcerns.find(
      (x) => x.Prediction == Math.max(...CosmeticData)
    );
    // let Null = data.CosmeticConcerns.find((data) => data.Name == "None");
    let Age = data.Demographics.Age;
    let Criteria = [];
    Criteria.push(Age.Criteria);
    ...
    if (
      ST2.includes(skindata.SkinType) &&
      (Cosmetics.Name == "Crow's Feet Wrinkles" ||
        Cosmetics.Name == "Forehead Rhytid" ||
        Cosmetics.Name == "Glabellar Rhytid")
    ) {
      let obj = {
        Name: "Wrinkling",
        SkinType: skindata,
    ...
    if (
      ...
      (Cosmetics.Name == "Crow's Feet Wrinkles" ||
        Cosmetics.Name == "Forehead Rhytid" ||
        Cosmetics.Name == "Glabellar Rhytid") &&
      (skindata2.SkinType == "I" || skindata2.SkinType == "II")
    ) {
      let obj = {
        Name: "Sun Damage",
    ...
    if (
      ((Criteria.includes("40-49") ||
        Criteria.includes("50-59") ||
        Criteria.includes("60-69") ||
        Criteria.includes("70+")) &&
        (Cosmetics.Name == "Crow's Feet Wrinkles" ||
          Cosmetics.Name == "Forehead Rhytid" ||
          Cosmetics.Name == "Glabellar Rhytid")) ||
      Cosmetics.Name == "Nasolabial lines"
    ) {
      let obj = {
        Name: "Signs Of Aging",
        Age:
...
exports.LogicalData = async (req, res) => {
  try {
    const userAnalysis = await Analysis.findOne({ User: req.user.userId });
    const Cosmetics = userAnalysis.CosmeticConcerns.filter(
      (x) => x.Detected == "True"
    );
    let Obj = {
      Name: req.body.name,
      Age: userAnalysis.Demographics.Age.Criteria,
      CosmeticData: Cosmetics,
      color: userAnalysis.skincolor,
      type: userAnalysis.skintype,
    };
    return res.status(200).json({ code: 200, message: "Res send", Obj });
  } catch (error) {
    return res.status(500).json({ code: 500, message: error.message });
  }
};
